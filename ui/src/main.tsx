...
function App() {
  ...
  return <div className="app">
    <aside>
      ...
      <div className="asidefoot"><span className="footer-orb"/> ASYGAMES NETWORK<br/><small>AsySounds · v0.1.5</small></div>
    </aside>
    <main>
      ...
      {view === 'Microphone' && <section className="voice-panel simple-voice-panel">
        ...
        <section className="voice-tuning" aria-label="Voice cleanup and clarity">
          <div className="tuning-card"><div className="tuning-head"><div><span className="eyebrow">TRANSIENT CONTROL</span><h3>Clap & impact filter</h3></div><strong>{impactStrength}%</strong></div><input type="range" min="0" max="100" step="1" value={impactStrength} aria-label="Clap and impact suppression" onChange={event => { setImpactStrength(Number(event.target.value)); setBypass(false); }}/><small>Targets brief clicks locally and isolated claps between phrases. To protect your voice, broad claps overlapping speech are not hard-muted; complete removal would need more capable source separation. Compare WAV stages and keep Clarity moderate. {preview.running ? "Impacts detected: " + preview.impact_events : ""}</small></div>
